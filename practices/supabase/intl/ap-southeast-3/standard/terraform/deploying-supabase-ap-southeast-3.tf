...
variable "ecs_flavor" {
  default     = "x1.8u.16g"
  description = "ECS instance flavor. x1.8u.16g (8vCPUs 16GiB) or above recommended. Supabase runs ~10 Docker containers."
  type        = string
  nullable    = false
  validation {
    condition     = can(regex("^x1\\.([1-9]|1[0-6])u\\.([1-9][0-9]{0,1}|1[0-2][0-8])g$", var.ecs_flavor))
    error_message = "Invalid flavor format, e.g. x1.8u.16g"
  }
}
...
variable "db_password" {
  default     = ""
  description = "PostgreSQL database password, 8-26 chars. Used as password for all database roles."
  type        = string
  sensitive   = true
  nullable    = false
}
...
variable "system_disk_size" {
  default     = 100
  description = "System disk size (GB), range: 40-1024. Supabase recommends 100GB or above. Default: 100."
  type        = number
  nullable    = false
  validation {
