...
> **文档类型：** 华为云解决方案实践部署指南
> **模板状态：** 候选 `deploying-jiuwenswarm_v4.tf`，应用版本 `0.2.2`
> **适用区域：** 中国站，华北-北京四（`cn-north-4`）
> **发布日期：** 2026-07-14

> [!IMPORTANT]
> 本方案尚未完成华为云 RFS 实际部署验证，也没有正式“开始部署”入口。候选模板通过云上测试并获得用户确认后，才能提升为正式模板；本文档当前用于候选评审和云测。
...
- **版本明确**：候选模板直接安装固定 JiuwenSwarm 版本 `0.2.2`。
...
- 上游说明要求 Python 3.11 至 3.13；模板使用 Ubuntu 24.04 的 `python3` 创建虚拟环境。实际解释器版本须在云测中确认。
...
5. 仅将候选文件 `practices/openjiuwen/cn/cn-north-4/jiuwenswarm/terraform/deploying-jiuwenswarm_v4.tf` 用于受控云测。

### 3.2 快速部署

1. 登录华为云控制台，进入资源编排服务 RFS，在华北-北京四创建资源栈。
2. 上传候选 Terraform 模板，创建执行计划并检查预计创建的资源、规格和费用。
3. 配置全部 8 个标准变量：
...
部署验证通过后，在 Web 界面按上游指引配置模型提供商。JiuwenSwarm 支持华为云 MaaS 以及多种 OpenAI 兼容接口；具体可用提供商和参数以候选版本 `0.2.2` 的界面为准。
...
| 正式 RFS 入口 | 未生成 |
| 云上部署验证 | 未执行 |
| 可上架状态 | 否；须完成云测、安全复核、文档人工审核并获得用户确认 |
