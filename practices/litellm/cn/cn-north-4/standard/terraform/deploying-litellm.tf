...
variable "ecs_flavor" {
  ...
  nullable    = false
  validation {
    condition     = can(regex("^[a-z][0-9a-z]*\\.([1-9]|[1-5][0-9]|6[0-4])u\\.([1-9]|[1-9][0-9]|1[0-2][0-8])g$", var.ecs_flavor))
    error_message = "Invalid ECS flavor format. Examples: x1.2u.4g, s6.4u.16g, c7.8u.32g"
  }
}
...
variable "db_password" {
  default     = ""
  description = "PostgreSQL password, 8-26 chars. Used by LiteLLM for internal database (virtual keys, spend tracking). Can be same as or different from ECS password."
  type        = string
  sensitive   = true
  nullable    = false
}
...
resource "huaweicloud_networking_secgroup_rule" "litellm_api" {
  security_group_id = huaweicloud_networking_secgroup.secgroup.id
  description       = "LiteLLM Proxy API and Admin UI (OpenAI-compatible endpoint)"
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  ports             = 4000
...
resource "huaweicloud_compute_instance" "compute_instance" {
  ...
  user_data = <<-USERDATA
    ...
    # ---- Install Docker CE (from Huawei Cloud mirror, China region) ----
    ...
    # Add your provider API keys below:
