...
resource "huaweicloud_networking_secgroup_rule" "openhands_web" {
  security_group_id = huaweicloud_networking_secgroup.secgroup.id
  description       = "OpenHands Web UI port"
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  ports             = 3000
