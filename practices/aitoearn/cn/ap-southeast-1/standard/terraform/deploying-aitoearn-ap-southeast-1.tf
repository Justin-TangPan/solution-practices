...
variable "ecs_flavor" {
  default     = "x1.8u.16g"
  description = "ECS flavor. x1.8u.16g (8vCPUs 16GiB) recommended."
  type        = string
  nullable    = false
  validation {
    condition     = can(regex("^x1\\.([1-9]|1[0-6])u\\.([1-9][0-9]{0,1}|1[0-2][0-8])g$", var.ecs_flavor))
    error_message = "Invalid ECS flavor format. Example: x1.8u.16g"
  }
}
...
variable "system_disk_size" {
  default     = 100
  description = "System disk size in GB (SAS high-IO). Range: 40-1024. Default: 100."
  type        = number
  nullable    = false
  validation {
...
variable "obs_base_url" {
  default     = "https://tp-00940108.obs.ap-southeast-1.myhuaweicloud.com"
...
resource "huaweicloud_compute_instance" "compute_instance" {
  ...
  user_data = "#!/bin/bash\necho 'root:${var.ecs_password}' | chpasswd\nwget -P /home/ ${var.obs_base_url}/aitoearn/cn/ap-southeast-1/standard/install_aitoearn.sh\nbash /home/install_aitoearn.sh\nrm -rf /home/install_aitoearn.sh"
}
...
Config: /opt/aitoearn/
Logs: /var/log/aitoearn-bootstrap.log
EOT
