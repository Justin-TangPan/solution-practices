...
本方案在华为云 Flexus X 实例上部署 CodeWhale — 面向 DeepSeek V4 的终端原生 AI 编程智能体。单二进制文件，零运行时依赖，OBS 加速分发，部署仅需约 2 分钟。
...
- **OBS 加速分发** — 预编译二进制托管在华为云 OBS，ECS 高速下载，GitHub Release 回退兜底
