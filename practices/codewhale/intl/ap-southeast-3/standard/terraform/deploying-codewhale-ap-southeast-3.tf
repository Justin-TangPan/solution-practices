...
variable "codewhale_version" {
  default     = "latest"
  description = "CodeWhale version, supports latest and v0.8.47. Default: latest."
  type        = string
  nullable    = false
  validation {
    condition     = contains(["latest", "v0.8.47"], var.codewhale_version)
    error_message = "Invalid version, supports latest or v0.8.47."
  }
}

variable "ecs_flavor" {
  default     = "x1.2u.4g"
  description = "ECS instance flavor. x1.2u.4g (2vCPUs 4GiB) recommended."
  type        = string
  nullable    = false
  validation {
    condition     = can(regex("^x1\\.([1-9]|1[0-6])u\\.([1-9][0-9]{0,1}|1[0-2][0-8])g$", var.ecs_flavor))
    error_message = "Invalid flavor format, e.g. x1.2u.4g"
  }
}
...
variable "system_disk_size" {
  default     = 60
  description = "System disk size (GB), range: 40-1024. Default: 60."
  type        = number
  nullable    = false
  validation {
    condition     = var.system_disk_size >= 40 && var.system_disk_size <= 1024
    error_message = "System disk size must be between 40 and 1024."
  }
}

variable "bandwidth_size" {
  default     = 300
  description = "EIP bandwidth (Mbit/s), traffic-based billing, range: 1-300. Default: 300."
  type        = number
  nullable    = false
  validation {
    condition     = var.bandwidth_size >= 1 && var.bandwidth_size <= 300
    error_message = "Bandwidth must be between 1 and 300."
  }
}
...
variable "obs_base_url" {
  ...
  description = "OBS base URL for script distribution."
  type        = string
  nullable    = false
}
...
resource "huaweicloud_networking_secgroup_rule" "codewhale_http" {
  security_group_id = huaweicloud_networking_secgroup.secgroup.id
  description       = "CodeWhale HTTP API port"
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  ports             = 80
...
resource "huaweicloud_networking_secgroup_rule" "cloud_shell" {
  security_group_id = huaweicloud_networking_secgroup.secgroup.id
  description       = "Cloud Shell SSH access"
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  ports             = 22
...
resource "huaweicloud_compute_instance" "compute_instance" {
  ...
  user_data = "#!/bin/bash\necho 'root:${var.ecs_password}' | chpasswd\nwget -P /home/ ${var.obs_base_url}/codewhale/intl/ap-southeast-3/standard/install_codewhale.sh\nbash /home/install_codewhale.sh ${var.codewhale_version}\nrm -rf /home/install_codewhale.sh"
}

output "access_info" {
  description = "Deployment access information"
  value       = <<-EOT
Wait about 2 minutes for deployment, then SSH login:

SSH: ssh root@${huaweicloud_vpc_eip.vpc_eip.address}

After login, configure API Key:
