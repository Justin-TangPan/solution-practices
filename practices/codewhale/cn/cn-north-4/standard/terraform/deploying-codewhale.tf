...
variable "codewhale_version" {
  default     = "latest"
  description = "CodeWhale 版本，支持 latest 及 v0.8.47 等。默认：latest。"
  type        = string
  nullable    = false
  validation {
    condition     = contains(["latest", "v0.8.47"], var.codewhale_version)
    error_message = "版本格式无效，支持 latest 或 v0.8.47。"
  }
}
...
resource "huaweicloud_networking_secgroup_rule" "codewhale_http" {
  security_group_id = huaweicloud_networking_secgroup.secgroup.id
  description       = "CodeWhale HTTP API端口"
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  ports             = 80
...
resource "huaweicloud_compute_instance" "compute_instance" {
  ...
  user_data = "#!/bin/bash\necho 'root:${var.ecs_password}' | chpasswd\nwget -P /home/ ${var.obs_base_url}/codewhale/cn/cn-north-4/standard/install_codewhale.sh\nbash /home/install_codewhale.sh ${var.codewhale_version}\nrm -rf /home/install_codewhale.sh"
}

output "access_info" {
  description = "部署访问信息"
  value       = <<-EOT
等待约2分钟部署完成，然后 SSH 登录：
