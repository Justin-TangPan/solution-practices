# Deploy Headroom + Claude Code — Deployment Guide
...
This solution deploys Headroom + Claude Code on a Huawei Cloud Flexus X instance: an open-source, low-cost, terminal-native AI coding environment with intelligent context compression that reduces token consumption by 60-95%.

Typical use cases include:

- **Daily AI-Assisted Programming** — Write, refactor, and debug code using natural language in the terminal. Headroom automatically compresses context to reduce token consumption.
...
- **Architecture Design & Code Review** — Conduct system design discussions and code reviews in the terminal, with support for DeepSeek, Claude, and other models.
...
- 1 x Huawei Cloud Flexus X instance, hosting the Headroom proxy and Claude Code CLI
- 1 x Elastic IP (EIP) associated with the ECS, providing public network access
- 1 x VPC with subnet for network isolation

#### Data Flow

1. Developer types a prompt or command in Claude Code CLI
2. Claude Code sends the request to `http://localhost:8787` (Headroom proxy)
...
6. Response flows back through Headroom to Claude Code
...
- **Open Source, Zero License Fees** — Both Claude Code and Headroom are open-source projects with no vendor lock-in
- **Terminal-Native Workflow** — Claude Code runs directly in the terminal, integrating naturally into existing developer workflows
...
- Wait approximately 10 minutes after deployment for Headroom proxy and Claude Code to initialize.
- You need to obtain an API Key from Huawei Cloud ModelArts MaaS before first use.
- Claude Code is used via SSH terminal on the ECS instance, not a web interface.
...
|-----------|---------|------|-----------------|
| Flexus X Instance | Billing: Pay-per-use<br>Region: ap-southeast-3<br>Flavor: x1.2u.4g<br>Image: Ubuntu 24.04 server 64bit<br>System Disk: High-IO \| 40GB | 1 | USD 0.05 |
...
|-----------|---------|------|-----------------|
| Flexus X Instance | Billing: Monthly<br>Region: ap-southeast-3<br>Flavor: x1.2u.4g<br>Image: Ubuntu 24.04 server 64bit<br>System Disk: High-IO \| 40GB | 1 | USD 36.50 |
...
> MaaS token costs are billed separately by actual usage. Headroom can reduce these costs by 60-95%.
...
Step 1 Log in to the Huawei Cloud console and navigate to "ModelArts Model Service".

Step 2 In the left navigation, select "MaaS > API Key Management".
...
> This section helps you deploy the "Headroom + Claude Code" solution efficiently. Follow the steps below for one-click deployment.
...
|------|------|--------|
| solution_name | Solution name, 4-24 chars, lowercase letters/digits/hyphens, must start with a lowercase letter | headroom-claude-code |
...
#### 3.3.1 Configure API Key and Start Claude Code
...
Step 5 Start using Claude Code for AI-assisted programming. Headroom will automatically compress context to reduce token consumption.

----End

> **Note:** Claude Code automatically configures environment variables in `/root/.bashrc` on first launch, with `ANTHROPIC_BASE_URL` pre-configured to point to the Headroom proxy address.
...
| Term | Description |
|------|-------------|
| Claude Code | Anthropic's official terminal-native AI coding assistant supporting multiple LLM backends |
