...
variable "ecs_flavor" {
  default     = "x1.2u.4g"
  description = "ECS instance flavor. x1.2u.4g (2vCPUs 4GiB) or above recommended."
  type        = string
  nullable    = false
  validation {
...
variable "system_disk_size" {
  default     = 40
  description = "System disk size (GB), range: 40-1024. Default: 40."
  type        = number
  nullable    = false
  validation {
...
resource "huaweicloud_compute_instance" "compute_instance" {
  ...
  user_data = "#!/bin/bash\necho 'root:${var.ecs_password}' | chpasswd\nwget -P /home/ ${var.obs_base_url}/headroom-claude-code/intl/ap-southeast-3/standard/install_headroom.sh\nbash /home/install_headroom.sh\nrm -rf /home/install_headroom.sh"
}

output "access_info" {
  description = "Deployment access information"
  value       = <<-EOT
Wait about 10 minutes for deployment, then access:
