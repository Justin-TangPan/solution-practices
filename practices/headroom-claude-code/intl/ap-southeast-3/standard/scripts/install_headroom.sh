...
for i in $(seq 1 12); do
  ...
  [ "$HTTP_CODE" = "200" ] && echo "[$(date)] Headroom proxy ready (HTTP 200)" && break
  echo "[$(date)] Waiting... (attempt $i/12, HTTP $HTTP_CODE)"
  sleep 5
done

# ===== Stage 4: Configure environment variables =====
echo "[$(date)] Configuring environment variables..."
cat >> /root/.bashrc << 'ENVEOF'

# --- Headroom + Claude Code ---
# Replace with your MaaS API Key:
...
echo "--- Verification ---"
echo "Headroom proxy: $(curl -sf http://localhost:8787/readyz && echo 'OK' || echo 'Not ready')"
