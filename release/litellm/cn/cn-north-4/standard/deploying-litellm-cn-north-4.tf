...
variable "ecs_flavor" {
  default     = "x1.8u.16g"
  description = "ECS flavor. Recommended x1.8u.16g (8vCPUs 16GiB) for production workloads."
  type        = string
  nullable    = false
  validation {
...
resource "huaweicloud_compute_instance" "compute_instance" {
  ...
  user_data = "#!/bin/bash\necho 'root:${var.ecs_password}' | chpasswd\nwget -P /home/ https://documentation-samples-5.obs.cn-north-4.myhuaweicloud.com/solution-as-code-publicbucket/solution-as-code-module/litellm-unified-ai-management-gateway/zh-cn/standard/install_litellm.sh\nbash /home/install_litellm.sh \"${var.db_password}\" \"${var.master_key}\" \"${var.salt_key}\"\nrm -rf /home/install_litellm.sh"
}
