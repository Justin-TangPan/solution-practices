...
import { ArrowRight, FileCode2, FileText, PackageOpen, type LucideIcon } from "lucide-react"
import { getDeployablePractices } from "@/lib/deployable"
import { getPractices } from "@/lib/catalog"

const formal = new Set(getPractices().map(practice => practice.slug))

function countFiles(root: string) {
  const counts = { terraform: 0, docs: 0, extension: 0 }
...
export default function AssetsPage() {
  ...
  const totals = practices.reduce((sum, practice) => ({ terraform: sum.terraform + practice.counts.terraform, docs: sum.docs + practice.counts.docs, extension: sum.extension + practice.counts.extension }), { terraform: 0, docs: 0, extension: 0 })
  const stats: [string, number, LucideIcon][] = [["Terraform", totals.terraform, FileCode2], ["Markdown", totals.docs, FileText], [".extension", totals.extension, PackageOpen]]

  return <div className="px-5 md:px-10 py-10 md:py-14 max-w-7xl mx-auto space-y-8">
    <header className="border-b border-border pb-8"><div className="flex flex-wrap items-center gap-3"><span className="eyebrow">Asset Management</span><span className="rounded-full border border-border bg-surface px-2 py-1 text-[0.65rem] text-ink-muted">只读 · 文件系统快照</span></div><h1 className="heading-lg text-ink mt-3">资产管理</h1><p className="text-sm text-ink-faded mt-3 max-w-2xl leading-relaxed">按正式方案盘点 Terraform、Markdown 文档与扩展配置；正式交付物是本地 release 归档和校验和。</p></header>

    <section className="grid grid-cols-1 gap-3 md:grid-cols-3">{stats.map(([label, value, Icon]) => <div key={label} className="rounded-xl border border-border bg-surface p-5 shadow-soft"><Icon className="h-4 w-4 text-accent" /><div className="eyebrow mt-4">{label}</div><div className="serif mt-2 text-3xl font-bold text-ink">{value}</div></div>)}</section>
    ...
    <section className="space-y-4">{practices.map(practice => <article key={practice.slug} className="rounded-2xl border border-border bg-surface p-6 shadow-soft">
      ...
      <div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-3">{Object.entries(practice.counts).map(([type, count]) => <div key={type} className="rounded-xl bg-cream p-4"><div className="eyebrow">{type}</div><div className="serif mt-2 text-2xl font-bold text-ink">{count}</div></div>)}</div>
    </article>)}</section>
  </div>
}
