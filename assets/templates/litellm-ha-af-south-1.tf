...
variable "resource_name_prefix" {
  default     = "ha-litellm"
  description = "资源名称前缀，命名规则为{resource_name_prefix}-资源英文名称。取值范围：4-24个字符，支持小写字母、数字、-（中划线），且必须以小写字母开头。"
  type        = string
  nullable    = false
...
variable "bandwidth_size" {
  default     = "300"
  description = "弹性公网带宽大小，按流量计费。单位：Mbit/s，取值范围：1-300。默认300Mbps。"
  type        = string
  nullable    = false
}
...
variable "litellm_version" {
  default     = "1.87.0"
  description = "LiteLLM Proxy 镜像版本号。模板会自动拼接为 docker.litellm.ai/berriai/litellm:{version}。"
  type        = string
  nullable    = false
}
...
variable "litellm_master_key" {
  default     = ""
  description = "LiteLLM 主密钥，必须以 sk- 开头。对外调用 Proxy 时使用的 Bearer Token。"
  type        = string
  nullable    = false
  sensitive   = true
  ...
  validation {
    ...
    error_message = "litellm_master_key 不能为空，且必须以 sk- 开头，最长24位。"
  }
}

variable "litellm_salt_key" {
  default     = ""
  description = "LiteLLM Salt Key，用于加密数据库中的敏感数据。建议使用8位以上随机字符串。最长24位"
  type        = string
  nullable    = false
  sensitive   = true
  ...
  validation {
    ...
    error_message = "litellm_salt_key 至少需要8个字符，最长24位"
  }
}

variable "cce_cluster_flavor" {
  default     = "cce.s2.small"
  description = "CCE Turbo 集群规格。默认为cce.s2.small(小规模多控制节点CCE集群，最大50节点)。"
  type        = string
  nullable    = false
...
variable "cce_node_pool_password" {
  default     = ""
  description = "CCE 集群节点密码，用于集群节点登录。"
  type        = string
  nullable    = false
  sensitive   = true
  validation {
...
variable "cce_node_pool_flavor" {
  ...
  description = "CCE 集群节点云服务器实例规格。建议至少满足 LiteLLM 官方 4C8G/实例的生产建议。"
  type        = string
  nullable    = false
}
...
variable "rds_flavor" {
  ...
  description = "RDS for PostgreSQL 主备版实例规格。默认 8U16G (独享型)"
  type        = string
  nullable    = false
}
...
variable "redis_capacity" {
  default     = 8
  description = "分布式缓存服务 Redis 实例规格。可选值：1GB-64GB，默认8GB。"
  type        = number
  nullable    = false
...
output "说明" {
  ...
  value      = "资源部署完毕后，可通过 http://${huaweicloud_vpc_eip.eips[0].address}/ui 访问 LiteLLM Proxy。"
}
