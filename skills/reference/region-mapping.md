...
| 区域组 | 包含的区域 | 特点 |
|--------|-----------|------|
| `cn` | cn-north-4, cn-southwest-2 | 国内站点，中文文档，Docker daemon 通过 `docker.wangzhou3.top` registry mirror 加速 + 华为云 apt 源 |
...
| Docker 镜像 | compose/install 中保留官方 Docker Hub 镜像名，通过 daemon `registry-mirrors` 使用 `docker.wangzhou3.top` | 官方 Docker Hub / `ghcr.io`（无前缀） |
| LiteLLM 镜像 | Docker Hub 镜像保留官方名并走 daemon mirror；`ghcr.io` 镜像需原样保留或使用已验证正式仓库 | `ghcr.io/berriai/litellm:main-stable` |

> ℹ️ **user_data 风格**：SAC 标准实践默认采用**全内联 user_data**（heredoc 生成 docker-compose/config + `docker compose up`），参考 LiteLLM、Supabase，不再默认走 OBS 拉脚本。

## OBS 脚本分发（例外模式）

只有用户明确要求或技术约束必须外置脚本时，RFS 模板的 `user_data` 才在 ECS 首次开机时从 OBS 拉取安装脚本，实现"脚本与模板解耦"。
...
2. **例外脚本模式下，`user_data` 里的脚本相对路径必须与桶内实际对象 key 一致。** 如果仓库采用 `scripts/` 层，路径也必须包含：
   ```
   ${obs_base_url}/<practice>/<site>/<region>/<standard|ha>/scripts/install_*.sh
   ```
   禁止写成与实际对象 key 不一致的路径，或 `litellm-hk/install_*.sh`（hk 当站点遗留）。
