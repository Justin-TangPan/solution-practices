...
**Solution as Code (SAC)** 是一套基于 AI Agent 的华为云解决方案自动化交付框架——将方案评估、脚本生成、文档撰写、打包发布全流程编码为可复用的技能包（Skill），通过 A2A 多智能体协同架构，实现从"需求输入"到"一键部署"的端到端自动化。
...
│                                                               │
│  ┌─────────────────────────┐  ┌─────────────────────────────┐ │
│  │huaweicloud-rfs-practices│  │    sac-doc-framework        │ │
│  │                         │  │                             │ │
...
| Skill | 核心职责 | 关键输出 |
|-------|----------|----------|
| **huaweicloud-rfs-practices** | RFS 模板工程的全部实战知识，包含 11 条踩坑反模式 | Terraform 模板规范、Shell 脚本 4 阶段模式、OBS 上传规范 |
...
`huaweicloud-rfs-practices` Skill 维护了 **11 条经过实战验证的反模式**，Agent 在生成脚本时必须遵守：
...
| 术语 | 全称 | 说明 |
|------|------|------|
| SAC | Solution as Code | 方案即代码，本框架的核心理念 |
...
| huaweicloud-rfs-practices | `~/.claude/skills/huaweicloud-rfs-practices/SKILL.md` | ~500 |
